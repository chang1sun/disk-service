...
func FileUploadHandler(w http.ResponseWriter, r *http.Request, pathParams map[string]string) {
	...
	if err != nil {
		util.ErrorResp(errcode.ParseHTTPRequestFormFileErrCode, errcode.ParseHTTPRequestFormFileErrMsg, err, &w)
		return
	}
	err = r.ParseForm()
	if err != nil {
	...
	if err := checkUserMd5Repeat(r.Context(), userID, md5); err != nil {
		...
		util.ErrorResp(uint32(s.Code()), s.Message(), nil, &w)
		return
	}
	...
	if fileMd5 != md5 {
		util.ErrorResp(errcode.Md5CheckNotPassCode, errcode.Md5CheckNotPassMsg, nil, &w)
		return
	}
...
func mergeFile(ctx context.Context, w http.ResponseWriter, md5 string, userID string) {
	...
	if res.Err() != nil {
		util.ErrorResp(errcode.DatabaseOperationErrCode, errcode.DatabaseOperationErrMsg, res.Err(), &w)
	}
	data := res.Val()
